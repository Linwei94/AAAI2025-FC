...
if __name__ == "__main__":
    ...
    if not os.path.exists(f'pre_calculated_logits/{args.dataset}_{args.model_name}.pt'):
        logits_val, labels_val, features_val = get_logits_labels(val_loader, net, return_feature=True)
        ...
        }, f'pre_calculated_logits/{args.dataset}_{args.model_name}.pt')

    # load logits, feature and labels
    data = torch.load(f'pre_calculated_logits/{args.dataset}_{args.model_name}.pt')
    ...
    # fc_cal.feature_clip=args.feature_clamp
    ...
    print(f"FC_GC: ECE={round(ece_fc_gc*100, 2)}, Accuracy={round(accuracy_fc_gc*100, 2)}")
